...
async function downloadImageWithCurl(url, filepath1x, filepath2x) {
  try {
    const result = execSync(`curl -s -L --max-time 30 "${url}"`, { encoding: 'buffer', maxBuffer: 10 * 1024 * 1024 });
...
function downloadAndProcessImage(url, userId) {
  return new Promise((resolve) => {
    ...
    request.on('error', async (err) => {
      ...
      if (err.code === 'EAI_AGAIN' || (err.message && err.message.includes('EAI_AGAIN'))) {
...
async function saveReview(review, outputDir, source = 'google') {
  ...
  const ratingDisplay = source === 'facebook'
    ? (review.rating === 5 ? 'recommended' : 'not recommended')
...
function updateLastFetched(business) {
